...
export default function DoctorCard({ doctor }) {
  ...
  const completedSessions = doctor.completedSessions || 0
      ...
      if (i < fullStars) {
        ...
        stars.push(
          <svg key={i} className="w-4 h-4 text-yellow-400" viewBox="0 0 20 20">
            <defs>
              <linearGradient id={`half-${doctor._id}`}>
                <stop offset="50%" stopColor="currentColor" />
                <stop offset="50%" stopColor="#e5e7eb" />
              </linearGradient>
            </defs>
            <path fill={`url(#half-${doctor._id})`} d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          </svg>
        )
      } else {
        stars.push(
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2">
      {/* Header with gradient background - Red theme for doctors */}
      <div className="bg-gradient-to-r from-red-500 to-red-600 h-24 sm:h-32 relative">
      ...
      <div className="pt-12 sm:pt-16 pb-4 sm:pb-6 px-4 sm:px-6 text-center">
        ...
        <p className="text-red-600 font-medium text-xs sm:text-sm mb-1">
        ...
        <div className="space-y-2 sm:space-y-3 mb-3 sm:mb-4">
          ...
          {/* Consultation Fee - Dynamic from doctor profile */}
          <div className="flex items-center justify-center bg-gray-50 px-3 sm:px-4 py-2 rounded-lg">
            ...
            <span className="text-xs sm:text-sm font-semibold text-gray-700">
              ₹{doctor.consultationFee || 499} Consultation
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-1 sm:gap-2 mb-3 sm:mb-4">
          ...
          <span className="bg-red-50 text-red-700 text-xs px-2 sm:px-3 py-1 rounded-full font-medium">
            🩺 Doctor
          </span>
          {doctor.isVerified && (
        ...
        {doctor.adminDisabled ? (
          ...
          <Link 
            href={`/book/${doctor._id}?type=doctor`}
            className="block w-full bg-gradient-to-r from-red-600 to-red-700 text-white font-semibold py-2 sm:py-3 px-4 sm:px-6 rounded-lg hover:from-red-700 hover:to-red-800 transition-all duration-300 shadow-md hover:shadow-lg text-sm sm:text-base"
          >
            Book Consultation
          </Link>
        )}
        ...
        <p className="text-xs text-gray-500 mt-2 sm:mt-3">
          {doctor.adminDisabled ? 'Not accepting bookings' : doctor.status === 'online' ? 'Available for online consultation' : 'Not available at the moment'}
        </p>
      </div>
    </div>
  )
