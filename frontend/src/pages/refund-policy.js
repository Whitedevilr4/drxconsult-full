...
export default function RefundPolicy() {
  ...
  const fetchLegalPage = async () => {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/website/legal/refund-policy`)
      setLegalPage(res.data)
      setLoading(false)
    } catch (err) {
      console.error('Error fetching refund policy:', err)
      setError(err.response?.status === 404 ? 'Refund policy not found' : 'Failed to load refund policy')
      setLoading(false)
    }
  }
      ...
      <Layout>
        <div className="max-w-4xl mx-auto py-12 px-4">
          <div className="bg-white rounded-lg shadow-md p-8 text-center">
            ...
            <p className="mt-2 text-gray-600">Loading refund policy...</p>
          </div>
        </div>
      </Layout>
    )
