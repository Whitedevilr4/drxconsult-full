...
export default function Disclaimer() {
  ...
  const fetchLegalPage = async () => {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/website/legal/disclaimer`)
      setLegalPage(res.data)
      setLoading(false)
    } catch (err) {
      console.error('Error fetching disclaimer:', err)
      setError(err.response?.status === 404 ? 'Disclaimer not found' : 'Failed to load disclaimer')
      setLoading(false)
    }
  }
      ...
      <Layout>
        <div className="max-w-4xl mx-auto py-12 px-4">
          <div className="bg-white rounded-lg shadow-md p-8 text-center">
            ...
            <p className="mt-2 text-gray-600">Loading disclaimer...</p>
          </div>
        </div>
      </Layout>
    )
