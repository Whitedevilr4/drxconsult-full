...
export default function TermsAndConditions() {
  ...
  const fetchLegalPage = async () => {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/website/legal/terms-and-conditions`)
      setLegalPage(res.data)
      setLoading(false)
    } catch (err) {
      console.error('Error fetching terms and conditions:', err)
      setError(err.response?.status === 404 ? 'Terms and conditions not found' : 'Failed to load terms and conditions')
      setLoading(false)
    }
  }
