...
export default function PharmacistDashboard() {
  ...
  const fetchPaymentStats = async (token) => {
    try {
      ...
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/pharmacists/payment-stats`, {
        headers: { Authorization: `Bearer ${token}` }
      })
  ...
  const fetchReviews = async (token) => {
    try {
      const pharmacist = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/pharmacists`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      
      // Get current pharmacist ID
      const user = typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('user') || '{}') : {}
      const currentPharmacist = pharmacist.data.find(p => p.userId?.email === user.email)
      
      if (currentPharmacist) {
        const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/bookings/reviews/${currentPharmacist._id}`)
        setReviews(res.data)
      }
    } catch (err) {
  ...
  return (
    <Layout>
      ...
      {!accessDenied && (
        <div className="fixed top-20 right-4 z-50">          <button            onClick={toggleOnlineStatus}
          ...
          <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8 text-center">
            <div className="mb-6">
              ...
              <p className="text-gray-600 mb-6">
                You don't have permission to access the pharmacist dashboard. This area is restricted to pharmacists and administrators only.
              </p>
            </div>
              ...
              <div className="flex items-end space-x-2 h-48">
                ...
                <div className="flex-1 flex flex-col items-center">
                  <div className="w-full bg-gray-100 rounded-t-lg overflow-hidden flex flex-col justify-end" style={{ height: '100%' }}>
                    <div 
                      className="bg-gradient-to-t from-green-500 to-green-400 w-full transition-all duration-500 flex items-center justify-center"
                      style={{ 
                        height: calculatedPaymentStats.totalEarned > 0 
                          ? `${(calculatedPaymentStats.totalPaid / calculatedPaymentStats.totalEarned) * 100}%`
                          : '0%',
                        minHeight: calculatedPaymentStats.totalPaid > 0 ? '30px' : '0px'
                      }}
                    >
                      {calculatedPaymentStats.totalPaid > 0 && (
                        <span className="text-xs font-bold text-white">₹{calculatedPaymentStats.totalPaid}</span>
                  ...
                  <p className="text-xs font-medium text-green-600 mt-2">Completed</p>
                  <p className="text-xs text-gray-500">{calculatedPaymentStats.paidBookings} bookings</p>
                </div>

                {/* Outstanding Payment Bar */}
                <div className="flex-1 flex flex-col items-center">
                  <div className="w-full bg-gray-100 rounded-t-lg overflow-hidden flex flex-col justify-end" style={{ height: '100%' }}>
                    <div 
                      className="bg-gradient-to-t from-orange-500 to-orange-400 w-full transition-all duration-500 flex items-center justify-center"
                      style={{ 
                        height: calculatedPaymentStats.totalEarned > 0 
                          ? `${(calculatedPaymentStats.outstanding / calculatedPaymentStats.totalEarned) * 100}%`
                          : '0%',
                        minHeight: calculatedPaymentStats.outstanding > 0 ? '30px' : '0px'
                      }}
                    >
                      {calculatedPaymentStats.outstanding > 0 && (
                        <span className="text-xs font-bold text-white">₹{calculatedPaymentStats.outstanding}</span>
                      )}
                    </div>
                  </div>
                  <p className="text-xs font-medium text-orange-600 mt-2">Outstanding</p>
                  <p className="text-xs text-gray-500">{calculatedPaymentStats.unpaidBookings} bookings</p>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'payments' && (
            <div>
              ...
              <div className="bg-white p-6 rounded-lg shadow-lg mb-6">
                <h3 className="text-lg font-bold text-gray-800 mb-4">Payment Visualization</h3>
                ...
                <div className="mb-6">
                  ...
                  <div className="w-full h-16 bg-gray-200 rounded-lg overflow-hidden flex">
                    {calculatedPaymentStats.totalPaid > 0 && (
                      <div 
                        className="bg-gradient-to-r from-green-500 to-green-600 flex items-center justify-center text-white font-bold transition-all duration-700"
                        style={{ 
                          width: `${(calculatedPaymentStats.totalPaid / calculatedPaymentStats.totalEarned) * 100}%`
                        }}
                      >
                        <span className="text-sm">Paid: ₹{calculatedPaymentStats.totalPaid}</span>
                      </div>
                    )}
                    {calculatedPaymentStats.outstanding > 0 && (
                      <div 
                        className="bg-gradient-to-r from-orange-400 to-orange-500 flex items-center justify-center text-white font-bold transition-all duration-700"
                        style={{ 
                          width: `${(calculatedPaymentStats.outstanding / calculatedPaymentStats.totalEarned) * 100}%`
                        }}
                      >
                        <span className="text-sm">Outstanding: ₹{calculatedPaymentStats.outstanding}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-8">
                  <div>
                    <p className="text-center text-sm font-medium text-gray-700 mb-3">Completed Payments</p>
                    <div className="flex flex-col items-center">
                      <div className="w-full max-w-xs bg-gray-100 rounded-t-lg overflow-hidden" style={{ height: '300px' }}>
                        <div className="h-full flex flex-col justify-end">
                          <div 
                            className="bg-gradient-to-t from-green-600 via-green-500 to-green-400 w-full transition-all duration-700 flex flex-col items-center justify-center"
                            style={{ 
                              height: calculatedPaymentStats.totalEarned > 0 
                                ? `${(calculatedPaymentStats.totalPaid / calculatedPaymentStats.totalEarned) * 100}%`
                                : '0%',
                              minHeight: calculatedPaymentStats.totalPaid > 0 ? '60px' : '0px'
                            }}
                          >
                            <span className="text-2xl font-bold text-white">₹{calculatedPaymentStats.totalPaid}</span>
                            <span className="text-sm text-white mt-1">{calculatedPaymentStats.paidBookings} bookings</span>
                          </div>
                        </div>
                      </div>
                      <div className="mt-3 text-center">
                        ...
                        <p className="text-xs text-gray-500">of total earnings</p>
                      </div>
                    </div>
                  </div>

                  <div>
                    <p className="text-center text-sm font-medium text-gray-700 mb-3">Outstanding Payments</p>
                    <div className="flex flex-col items-center">
                      <div className="w-full max-w-xs bg-gray-100 rounded-t-lg overflow-hidden" style={{ height: '300px' }}>
                        <div className="h-full flex flex-col justify-end">
                          <div 
                            className="bg-gradient-to-t from-orange-600 via-orange-500 to-orange-400 w-full transition-all duration-700 flex flex-col items-center justify-center"
                            style={{ 
                              height: calculatedPaymentStats.totalEarned > 0 
                                ? `${(calculatedPaymentStats.outstanding / calculatedPaymentStats.totalEarned) * 100}%`
                                : '0%',
                              minHeight: calculatedPaymentStats.outstanding > 0 ? '60px' : '0px'
                            }}
                          >
                            <span className="text-2xl font-bold text-white">₹{calculatedPaymentStats.outstanding}</span>
                            <span className="text-sm text-white mt-1">{calculatedPaymentStats.unpaidBookings} bookings</span>
                          </div>
                        </div>
                      </div>
                      <div className="mt-3 text-center">
                        <p className="text-lg font-bold text-orange-600">
                          {calculatedPaymentStats.totalEarned > 0 
                            ? Math.round((calculatedPaymentStats.outstanding / calculatedPaymentStats.totalEarned) * 100)
                            : 0}%
                        </p>
                  ...
                  {reviews.reviews.map((review) => (
                    <div key={review._id} className="bg-white p-6 rounded-lg shadow hover:shadow-md transition-shadow">
                      ...
                      <p className="text-xs text-gray-500 mt-2">
                        Session Date: {new Date(review.slotDate).toLocaleDateString()}
                      </p>
                    </div>
                  ))}
                </div>
                ...
                <div key={booking._id} className="bg-white p-6 rounded-lg shadow">
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex-1">
                      ...
                      <p className="text-sm">Date: {new Date(booking.slotDate).toLocaleDateString()}</p>
                      ...
                      <div className="flex items-center space-x-2 mt-2">
                        ...
                        {booking.status === 'completed' && (
                          <span className={`text-xs px-2 py-1 rounded font-medium ${
                            booking.pharmacistPaid
                              ? 'bg-green-100 text-green-700 border border-green-300'
                              : 'bg-yellow-100 text-yellow-700 border border-yellow-300'
                          }`}>
                            {booking.pharmacistPaid ? '💰 Payment Received' : '⏳ Payment Pending'}
                          </span>
                        )}
                      </div>
