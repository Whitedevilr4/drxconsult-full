...
export default function NutritionistDashboard() {
  ...
  useEffect(() => {
    ...
    if (!token || !userData.role) {
      router.push('/login?redirect=/nutritionist/dashboard')
      return
    }
    
    // Check if user has nutritionist or admin role
    if (userData.role !== 'nutritionist' && userData.role !== 'admin') {
  ...
  useEffect(() => {
    ...
    if (completedBookings.length > 0) {
      const totalEarned = completedBookings.reduce((sum, b) => sum + (b.nutritionistShare || 250), 0)
      const paidBookingsArray = completedBookings.filter(b => b.nutritionistPaid)
      const totalPaid = paidBookingsArray.reduce((sum, b) => sum + (b.nutritionistShare || 250), 0)
  ...
  const fetchBookings = async (token) => {
    try {
      ...
      // Fetch nutritionist profile to get online status
      try {
        const profileRes = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/nutritionists/profile`, {
          headers: { Authorization: `Bearer ${token}` }
        })
      ...
      const totalEarned = completedBookings.reduce((sum, b) => {
        const share = b.nutritionistShare || 225
        return sum + share
      }, 0)
      ...
      if (err.response?.status === 404) {
        console.log('No Nutritionist profile found - this is normal for new Nutritionists')
      } else {
  ...
  const fetchPaymentStats = async (token) => {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/Nutritionists/payment-stats`, {
        headers: { Authorization: `Bearer ${token}` }
      })
  ...
  const fetchReviews = async (token) => {
    try {
      const Nutritionist = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/Nutritionists`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      
      // Get current Nutritionist ID
      const user = typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('user') || '{}') : {}
      const currentNutritionist = Nutritionist.data.find(d => d.userId?.email === user.email)
      
      if (currentNutritionist) {
        const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/bookings/reviews/${currentNutritionist._id}?type=Nutritionist`)
        setReviews(res.data)
      }
    } catch (err) {
  ...
  const toggleOnlineStatus = async () => {
    try {
      ...
      await axios.patch(
        `${process.env.NEXT_PUBLIC_API_URL}/nutritionists/status`,
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      )
          ...
          <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8 text-center">
            <div className="mb-6">
              ...
              <p className="text-gray-600 mb-6">
                You don't have permission to access the Nutritionist dashboard. This area is restricted to Nutritionists and administrators only.
              </p>
            </div>
            ...
            <div className="mt-6 p-4 bg-yellow-50 rounded-lg">
              <p className="text-sm text-yellow-800">
                <strong>Are you a Nutritionist?</strong> Please contact the system administrator to get Nutritionist access.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className={`bg-gray-50 min-h-screen transition-all duration-300 ${!isOnline ? 'blur-sm pointer-events-none select-none' : ''}`}>
          <div className="container mx-auto px-4 py-8">
            <div className="flex justify-between items-center mb-6">
              <h1 className="text-3xl font-bold text-gray-800">Nutritionist Dashboard</h1>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
              <div className="bg-gradient-to-br from-green-500 to-green-600 p-4 rounded-lg shadow-lg text-white">
                <p className="text-green-100 text-xs font-medium">Total Patients</p>
            ...
            {/* Payment Stats Card */}
            <div className="bg-gradient-to-r from-green-50 to-green-50 p-6 rounded-lg shadow-lg mb-6 relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="bg-white p-4 rounded-lg shadow">
                  ...
                  <p className="text-3xl font-bold text-green-600">₹{calculatedPaymentStats.totalEarned}</p>
                  ...
                  <div className="bg-white p-6 rounded-lg shadow-lg border-l-4 border-green-500">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-bold text-green-600 text-xl">₹{calculatedPaymentStats.totalEarned}</span>
                      </div>
                ...
                <div className="bg-gradient-to-r from-yellow-50 to-orange-50 p-6 rounded-lg shadow-lg mb-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="bg-white p-4 rounded-lg shadow text-center">
                      ...
                      <p className="text-4xl font-bold text-green-600 my-2">{reviews.totalReviews}</p>
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-2 mt-2">
                          ...
                          {booking.status === 'completed' && (
                            <span className={`text-xs px-2 py-1 rounded font-medium ${
                              booking.nutritionistPaid
                                ? 'bg-green-100 text-green-700 border border-green-300'
                                : 'bg-yellow-100 text-yellow-700 border border-yellow-300'
                            }`}>
                              {booking.nutritionistPaid ? '💰 Payment Received' : '⏳ Payment Pending'}
                            </span>
                          )}
                        </div>
                        ...
                        {booking.status === 'completed' && (
                          <p className="text-sm text-gray-600 mt-1">
                            Your share: <span className="font-semibold text-gray-800">₹{booking.nutritionistShare || Math.round((booking.paymentAmount || 0) * 0.5)}</span>
                            <span className="text-xs text-gray-500 ml-1">
                              (from ₹{booking.paymentAmount || 0} consultation)
                            </span>
                            {booking.nutritionistPaid && booking.paidAt && (
                    ...
                    <div className="border-t pt-4 space-y-3">
                      ...
                      {booking.status === 'confirmed' && (
                        <div className="bg-green-50 p-3 rounded border border-green-200">
                          <p className="text-sm font-semibold text-green-900 mb-2">Requirements to Mark as Treated:</p>
                      ...
                      {booking.status === 'confirmed' && (
                        <button 
                          onClick={() => router.push(`/Nutritionist/upload-report/${booking._id}`)}
...
// Medical Forms Tab Component for Nutritionist (same as Pharmacist)
function MedicalFormsTabContent() {
  ...
  return (
    <div>
      ...
      {/* Statistics */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-gradient-to-r from-green-50 to-indigo-50 p-6 rounded-lg shadow">
