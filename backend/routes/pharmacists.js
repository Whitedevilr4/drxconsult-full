  ...
  try {
    ...
    for (const pharmacist of pharmacists) {
      if (pharmacist.availableSlots && pharmacist.availableSlots.length > 0) {
        try {
          await Promise.race([
            cleanupExpiredSlotsForPharmacist(pharmacist._id),
            new Promise((_, reject) => 
              setTimeout(() => reject(new Error('Cleanup timeout')), 2000)
            )
          ]);
        } catch (cleanupError) {
          console.error(`Cleanup failed for pharmacist ${pharmacist._id}:`, cleanupError);
          // Continue with other pharmacists even if cleanup fails
        }
      }
    }
    
    // Fetch again after cleanup and add rating/session data
    const updatedPharmacists = await Promise.race([
      Pharmacist.find().populate('userId', 'name email').maxTimeMS(5000),
      new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Database query timeout')), 5000)
      )
